...
def train_one_epoch(args,train_loader,model,optimizer,scheduler,criterion,epoch,K):
    ...
    for step, (sentence,label) in enumerate(epoch_iterator):
        # print(sentence,label)
        if args.gpu:
            sentence = sentence.cuda()
        if args.board and step == 0 and epoch == 1 and (K == 1 or K == 0):
            writer.add_graph(bilstm_attention.module,sentence)
...
def train(args,data,model,tokenizer):
    ...
    all_dataset = LoadData(data,word_dict)
    # loss function
    ...
    for epoch in range(args.epoch):
        ...
        if args.K >= 2:
            ...
            K_accuracy = test(args,test_data,bilstm_attention,1,1)
...
def test(args,data,model,tokenizer,sign):
    ...
    test_dataset = LoadData(data,word_dict)
...
if __name__ == '__main__':
    ...
    # build vocab
    word_dict = build_vocab(pd.concat([train_data, test_data], axis=0))
    ...
    bilstm_attention = BiLSTM_Attention(args,vocab_size,embedding_dim,n_hidden,num_classes)
    if args.load:
